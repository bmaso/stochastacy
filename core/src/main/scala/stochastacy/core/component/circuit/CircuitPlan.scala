 ...
 *  (e.g. `{ case Admit(r) => r }` lifted). Routes add no delay: the emitter's `Scheduled` delay is the latency. */
private[stochastacy] final case class Route(target: RouteTarget, transform: Any => Option[Any] = Route.accept)
 ...
 * route whose transform accepts it delivers a copy (fan-out). This is internal plumbing: the typed circuit builder
 ...
 * ports or the forward outlet; a node's `Consumption` plane goes only to the consumption outlet (feedback driven by
  ...
  routes.foreach { (source, rs) =>
    ...
    rs.foreach { r =>
      ...
      val permitted = (source, r.target) match
        case (RouteSource.CircuitInput, RouteTarget.NodePort(_, _))                            => true
        case (RouteSource.CircuitInput, _)                                                     => false
        case (RouteSource.NodePlane(_, CircuitPlane.Consumption), RouteTarget.ConsumptionOutlet) => true
        case (RouteSource.NodePlane(_, CircuitPlane.Consumption), _)                           => false
        case (RouteSource.NodePlane(_, _), RouteTarget.ConsumptionOutlet)                      => false
        case (RouteSource.NodePlane(_, _), _)                                                  => true
      require(permitted, s"route $source → ${r.target} is not a permitted circuit route")
    }
  }
