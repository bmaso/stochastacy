    ...
    new GraphStageLogic(shape):
      ...
      private def tryEmitNext(): Unit =
        if derivedQueue.nonEmpty then
          ...
          baseIter.head match
            ...
            case TimedControlEvent.EndOfTime if isWaitingForTickResponse =>
              // HOLD EndOfTime until the final tick's response has come back.
              // advanceTick for that response may still generate derived
              // (FollowOn) requests; emitting EndOfTime now would place business
              // elements AFTER the terminal sentinel — a timed-event-protocol
              // violation.  (DynamoDbTable happened to tolerate trailing
              // requests, masking this; a protocol-strict consumer such as
              // SystemBoundaryStage treats EndOfTime as terminal and silently
              // absorbs them, leaving inFlight stranded and wedging completion.)
              outletWaiting = true
              ensureResponsePulled()

            case _: TimedControlEvent =>
              // EndOfTime (once no tick response is pending — the derivedQueue
              // branch above has priority, so any derived requests generated by
              // the final advanceTick are flushed first) and any other control
      ...
      setHandler(responseIn, new InHandler:
        override def onPush(): Unit =
          ...
          // ALWAYS keep responseIn pulled until upstream finishes — not merely
          // while requestOut is open.  Two reasons:
          //  1. The cycle requires continuous demand on responseIn so the table
          //     keeps pulling its request inlet and firing onPull(requestOut).
          //  2. Liveness: this stage typically consumes responses via a Broadcast
          //     shared with other sinks.  If we stop pulling after completing
          //     requestOut, any element still behind us (late SDK-retry responses,
          //     the terminal EndOfTime) can never be delivered — Broadcast needs
          //     demand from ALL outlets — wedging every sibling sink and blocking
          //     upstream completion.  Draining post-completion is harmless: the
          //     elements only feed accounting that no longer emits anything.
          // Idempotent — ensureResponsePulled is a no-op if responseIn is
          // already pulled or closed.
          ensureResponsePulled()
