...
final class WorkloadFile private[workload] (
  ...
  def resolve(name: String): WorkloadTemplate =
    ...
    val (flows, bindings) = collectFlows(name, Vector.empty)
    new WorkloadTemplate(flows, bindings)

  private def collectFlows(
    name:    String,
    visited: Vector[String]
  ): (Vector[TemplateFlow], Set[String]) =
    ...
    val (incFlows, incBindings) = entry.include.foldLeft(
      (Vector.empty[TemplateFlow], Set.empty[String])
    ) { case ((flows, bindings), incName) =>
      val (f, b) = collectFlows(incName, newVisited)
      (flows ++ f, bindings ++ b)
    }

    val ownBindings = entry.flows.flatMap(indexVarsOf).toSet
    (incFlows ++ entry.flows, incBindings ++ ownBindings)
