      ...
      Iterator.single(TimedControlEvent.Tick(SimTime.of(tick)): TimedElement[DynamoDBRequest]) ++
        (0 until n).iterator.flatMap { i =>
          ...
          Iterator.fill(count) {
            ...
            buildRequest(tick, workload.usecase, workload.flows(i).shape, paramRngs(i), φ)
          }
        }
    } ++ Iterator[TimedElement[DynamoDBRequest]](
  ...
  private def buildRequest(
    ...
    shape match
      case RequestShape.GetItem =>
        GetItemRequest(t, usecase, intraTick)
      case RequestShape.DeleteItem =>
        DeleteItemRequest(t, usecase, intraTick)
      case RequestShape.PutItem(itemBytesSampler) =>
        PutItemRequest(t, usecase, itemBytesSampler.sample(tick, rng, ())._1, intraTick)
      case RequestShape.UpdateItem(itemBytesSampler) =>
        UpdateItemRequest(t, usecase, itemBytesSampler.sample(tick, rng, ())._1, intraTick)
      case RequestShape.Query(target, readConsistency) =>
        QueryRequest(t, usecase, target, readConsistency, intraTick = intraTick)
      case RequestShape.Scan(target, readConsistency) =>
        ScanRequest(t, usecase, target, readConsistency, intraTick = intraTick)
      case RequestShape.TransactWriteItems(perItemSamplers) =>
        TransactWriteItemsRequest(t, usecase, perItemSamplers.map(_.sample(tick, rng, ())._1), intraTick)
      case RequestShape.TransactGetItems(itemCountSampler) =>
        TransactGetItemsRequest(t, usecase, itemCountSampler.sample(tick, rng, ())._1, intraTick)
