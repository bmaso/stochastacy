...
class TimeWindowTicksSourceTest extends AnyWordSpecLike with should.Matchers with BeforeAndAfterAll:
  ...
  "A TimeWindowTicksSource" should:
    "be a valid timed event source" in:
      ...
      sub.request(10)
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(0L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(1L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(2L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(3L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(4L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(5L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(6L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(7L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(8L)))
      sub.expectNext(TimedControlEvent.Tick(SimTime.of(9L)))
