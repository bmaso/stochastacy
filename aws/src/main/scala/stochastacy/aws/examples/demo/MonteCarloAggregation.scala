...
object MonteCarloAggregation:
  ...
  /** The PITR continuous-backup cost — appended only when the ensemble enabled PITR (so non-PITR output is
   *  unchanged). */
  private val pitrSummaryMetrics: Vector[(String, TrialSummary => BigDecimal)] = Vector(
    ("TotalPitrCost", (s: TrialSummary) => s.totalPitrCost)
  )
  ...
  /** Whether the ensemble incurred any PITR cost (so the PITR metric is worth reporting). */
  def hasPitr(trials: Vector[TrialResult]): Boolean =
  ...
  def summaryMetrics(gsiNames: Vector[String], provisioned: Boolean = false, pitr: Boolean = false): Vector[(String, TrialSummary => BigDecimal)] =
    ...
    } ++ (if provisioned then provisionedSummaryMetrics else Vector.empty) ++ (if pitr then pitrSummaryMetrics else Vector.empty)
  ...
  private def aggregatorFor(trials: Vector[TrialResult]): IncrementalAggregator =
    ...
    val agg   = new IncrementalAggregator(timeSeriesMetrics(names), summaryMetrics(names, hasProvisioning(trials), hasPitr(trials)))
