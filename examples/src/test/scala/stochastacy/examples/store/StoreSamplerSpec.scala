...
class StoreSamplerSpec extends AnyWordSpec with should.Matchers:
  ...
  "StoreSampler point operations" should {
    ...
    "return a hit with mean-sized bytes, no state change, and completion-timed consumption" in {
      ...
      val e   = smp.sample(Get(t), s, rng())

      e.response.event shouldBe GetResult(hit = true, bytes = 2048L)
      e.newState shouldBe s
      // D-B: consumption is accounted at completion — same delay as the response.
      e.response.delay shouldBe cfg.pointLatency
    ...
    "return a miss with zero bytes and zero returned items" in {
      ...
      val e   = smp.sample(Get(t), StoreState(10L, 10L * 2048L), rng())
      e.response.event shouldBe GetResult(hit = false, bytes = 0L)
    ...
    "grow cardinality and bytes on a creating Put, emitting a positive StorageDelta" in {
      ...
      val e   = smp.sample(Put(2000L, t), StoreState(10L, 10L * 1000L), rng()) // meanBytes = 1000
      e.response.event shouldBe WriteResult(created = true)
    ...
    "keep cardinality but adjust bytes on an updating Put (delta = size - meanBytes)" in {
      ...
      val e   = smp.sample(Put(2000L, t), StoreState(10L, 10_000L), rng()) // meanBytes = 1000
      e.response.event shouldBe WriteResult(created = false)
    ...
    "shrink state on a hit Delete and emit a negative StorageDelta" in {
      ...
      val e   = smp.sample(Delete(t), StoreState(10L, 10_000L), rng()) // meanBytes = 1000
      e.response.event shouldBe DeleteResult(deleted = true)
    ...
    "leave state untouched on a miss Delete" in {
      ...
      val e   = smp.sample(Delete(t), s, rng())
      e.response.event shouldBe DeleteResult(deleted = false)
      e.newState shouldBe s
    }
  }
  ...
  "StoreSampler list queries" should {
    ...
    "climb evaluated cost with page depth under offset pagination (the deep-offset cliff)" in {
      ...
      def evaluated(pageIndex: Int): Long =
        queryOf(
          smp.sample(
            ListQuery(SelectivityClass.FullScan, SortMode.IndexOrdered, Pagination.Offset(pageIndex, 10), t),
            big,
            rng()
          )
        ).evaluatedItems
    ...
    "keep evaluated cost flat under keyset pagination regardless of matched size" in {
      ...
      def evaluated(entityCount: Long): Long =
        queryOf(
          smp.sample(
            ListQuery(SelectivityClass.FullScan, SortMode.IndexOrdered, Pagination.Keyset(10), t),
      ...
      def matched(entityCount: Long): Long =
        queryOf(
          smp.sample(
            ...
            ListQuery(SelectivityClass.PointLookup, SortMode.RequiresSort, Pagination.Keyset(10), t),
            StoreState(entityCount, entityCount * 1024L),
    ...
    "realize CategoryFilter as a ~constant fraction that scales with entity count" in {
      ...
      def matched(entityCount: Long): Long =
        queryOf(
          smp.sample(
            ListQuery(SelectivityClass.CategoryFilter, SortMode.RequiresSort, Pagination.Keyset(10), t),
            StoreState(entityCount, entityCount * 1024L),
            rng()
          )
        ).evaluatedItems
  ...
  "StoreSampler report queries" should {
    ...
    "evaluate the whole matched set but return only the (paged) group count" in {
      val smp = new StoreSampler(StoreConfig())
      val e = smp.sample(
        ReportQuery(SelectivityClass.FullScan, groupCount = 20, SortMode.RequiresSort, Pagination.Keyset(50), t),
  ...
  "StoreSampler" should {
    ...
    "take the error branch, emitting an ErrorResult with no consumption and no state change" in {
      ...
      val e   = smp.sample(Get(t), s, rng())
      e.response.event shouldBe ErrorResult("system")
    ...
    "grow entity count by one per creating Put when state is threaded across many requests" in {
      ...
      (1 to 1000).foreach { i =>
        st = smp.sample(Put(500L, SimTime.of(i.toLong)), st, r).newState
      }
